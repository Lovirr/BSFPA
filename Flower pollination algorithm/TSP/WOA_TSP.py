...
model = WOA.OriginalWOA(epoch)
best_position, best_fitness = model.solve(problem)
print(f"Best solution: {best_position}\nObj = Total Distance: {best_fitness}")

# 绘制收敛曲线
plt.figure(figsize=(8, 6), dpi=300)
plt.plot(model.history.list_global_best_fit, 'b', linewidth=2, label='WOA')
plt.title('WOA TSP Convergence curve: ', fontsize=15)
...
# 保存图片
TSP.plot_solutions(dict_solutions, filename=f"WOA-{filename}-epochs", pathsave="./results/WOA-TSP")
